...
const Textarea = ({ label, placeholder, required, value, rows = 4, name, onChange, error, ast }) => {
  ...
  return (
    <div className='flex flex-col gap-1.5 items-start'>
      {label && <label className="md:text-sm text-xs font-medium text-neutral-600">{label} {ast ? (<span className='text-red-500 text-xs'>*</span>) : ('')}</label>}
      <textarea
        className={className}
